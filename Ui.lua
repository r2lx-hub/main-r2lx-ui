...
function library:CreateWindow(props)
	-- // properties
	local name = props.name or props.Name or props.CreateWindow or props.Page or props.pagename or props.Pagename or props.PageName or props.pageName or "new ui"
	-- // variables
	local CreateWindow = {}
	-- // main
...
utility.new("UIListLayout", {
	...
	--
	table.insert(self.pages,CreateWindow)
	--
	button.MouseButton1Down:Connect(function()
		if CreateWindow.open == false then
			for i,v in pairs(self.pages) do
				if v ~= CreateWindow then
					if v.open then
						v.CreateWindow.Visible = false
						v.open = false
			...
			self:closewindows()
			--
			CreateWindow.CreateWindow.Visible = true
			CreateWindow.open = true
			CreateWindow.outline.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
			CreateWindow.line.Size = UDim2.new(1,0,0,3)
			CreateWindow.line.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
		end
	end)
	--
	...
	--
	if pointer then
		self.pointers[tostring(pointer)] = CreateWindow.pointers
	end
	--
	self.labels[#self.labels+1] = label
	-- // metatable indexing + return
	setmetatable(CreateWindow, pages)
	return CreateWindow
end
--
function pages:openpage()
	local CreateWindow = self
	--
	if CreateWindow.open == false then
		for i,v in pairs(CreateWindow.library.pages) do
			if v ~= CreateWindow then
				if v.open then
					v.CreateWindow.Visible = false
					v.open = false
...
function sections:toggle(props)
	...
	button.MouseButton1Down:Connect(function()
		if toggle.current then
			...
			toggle.color.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
